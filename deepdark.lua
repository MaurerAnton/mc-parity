...
-- ---------------------------------------------------------------------------
-- 3) Functional sculk shriekers (both games' shrieker logic is inert)
-- ---------------------------------------------------------------------------
local SHRIEK_COOLDOWN = 10

local function activate_shrieker(center)
	local s = minetest.find_node_near(center, 8, { SHRIEKER })
	if not s then
		return
	end
	local n = minetest.get_node(s)
	if n.param2 ~= 1 then
		minetest.sound_play("mcl_sculk", { pos = s, gain = 1.5, max_hear_distance = 16 }, true)
		n.param2 = 1
		minetest.set_node(s, n)
		minetest.after(SHRIEK_COOLDOWN, function()
			local nn = minetest.get_node(s)
			if nn.name == SHRIEKER then
				minetest.set_node(s, { name = SHRIEKER, param2 = 0 })
			end
		end)
	end
end

local function on_player_walk(pos, node, player)
	if not player then
		return
	end
	...
	if v and v.x == 0 and v.y == 0 and v.z == 0 then
		return
	end
	activate_shrieker(pos)
end

if minetest.get_modpath("walkover") then
	walkover.register_global(on_player_walk)
	minetest.log("action", "[mcl_mobs_addon] shrieker hook: walkover")
elseif minetest.get_modpath("mcl_walkover") then
	mcl_walkover.register_global(on_player_walk)
	minetest.log("action", "[mcl_mobs_addon] shrieker hook: mcl_walkover")
end
